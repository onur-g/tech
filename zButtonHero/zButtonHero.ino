...
int val3,val4,val5;                        // variable for reading the pin status
int buttonState3,buttonState4,buttonState5;                // variable to hold the last button state
...
void loop() {
  if (digitalRead(3)==HIGH || digitalRead(4)==HIGH || digitalRead(5)==HIGH){ // if any button is pressed
  ...
  if (started==true){ // if the game has been started
    ...
    if (val3 != buttonState3 && val3 == HIGH || val4 != buttonState4 && val4 == HIGH || val5 != buttonState5 && val5 == HIGH) {          // the button state has changed!
